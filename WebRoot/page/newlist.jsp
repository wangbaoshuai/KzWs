...
<script type="text/javascript">
	 function deletenews(nid){
	   if(confirm("是否确认要删除？"))
	   location.href="<%=basePath%>deleteNew.action?nid="+nid;
	   }
	...
	function openwin(){
	  ...
	  var paly =showModalDialog("page/newadd.jsp","_blank","dialogTop=50px;dialogWidth=500px;dialogHeight=500px;scroll=0;center=1;help=0");
	}
	</script>
...
<table width="1280" border="1">
   ...
   <s:iterator var="news" value="#session.newList" status="i">
   <tr <s:if test="#i.index %2==0 ">style="background:#FFF;"</s:if>>
   <td><s:property value="#news.NId"/></td>
   <td><s:property value="#news.NTitle"/></td>
   <td>
   <c:if test="${news.NContent.length() >= 25 }">${news.NContent.substring(0,25 )}...</c:if>
   <c:if test="${news.NContent.length() <25  }">${news.NContent}</c:if>
   </td>
   <td><img src="news_images/<s:property value="#news.NImage"/>" alt="" width="80" height="50"></td>
   <td><a href="findoneNew.action?nid=<s:property value="#news.NId"/>"  class="ico edit">*修改*</a>|
  <a href="javascript:deletenews(<s:property value="#news.NId"/>)" class="ico del">*删除*</a></td>
   </tr> 
   </s:iterator>
   </table>
  
   <div class="pagelist">
   <a href="findNew.action?type=houtai&nowpage=${backpage }">上一页</a>
    <c:forEach var="i" step="1" begin="1" end="${pages}">
    <a href="findNew.action?type=houtai&nowpage=${i}">${i}&nbsp;&nbsp;</a>
   </c:forEach>
   <a href="findNew.action?type=houtai&nowpage=${nextpage }">下一页</a>
   <a href="findNew.action?type=houtai&nowpage=${pages }">末页</a>
     <a>${nowpage}/${pages}页</a> 
   </div>
</div>
</div></div> 
