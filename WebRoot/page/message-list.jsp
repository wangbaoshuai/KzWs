...
<html>
  <head>
    ...
    <title>My JSP 'message-list.jsp' starting page</title>
...
<link type="text/css" rel="stylesheet" href="resource/css/framework.css" />
    ...
    <script type="text/javascript">
	 function deletemessage(id){
	   if(confirm("是否确认要删除？"))
	   location.href="deleteMessage.action?type=add&mid="+id;
	   }
	</script>
	<style type="text/css">
	#com{float:right;}
				...
				<div class="verticalMenu">
					<div class="ui vertical pointing menu fluid">
						...
						<a class="item" href="findByPageAndUserComment.action?user.UId=${loginUser.UId}&nowpage=1">
  						...
  						<a class="active teal item" href="findByPageAndUserMessage.action?user.UId=${loginUser.UId}&nowpage=1">
				...
				<div class="pageHeader">
					<div class="segment">
						<h3 class="ui dividing header">
  							...
  							<div class="content">
    							我的留言
    							<div class="sub header">我的留言以及回复</div>
  							</div>
						</h3>
					</div>
				</div>
				...
				<c:forEach var="message" items="${messageList}">
					<div class="event">
						...
						<div class="content">
							  ...
							  <div class="summary">
								 管理员 <a>对我的回复:</a>&nbsp;
								 <c:if test="${message.MReply.length() >= 20 }">${message.MReply.substring(0,20 )}...</c:if>
						        <c:if test="${message.MReply.length() <20  }">${message.MReply}</c:if>
		
							  </div>
							  <div class="extra text">
								<img src="images/format.gif" alt="" width="20" height="20">我的留言<img src="images/to.gif" alt="" >&nbsp;
								<c:if test="${message.MContent.length() >= 24 }">${message.MContent.substring(0,24 )}...</c:if>
						        <c:if test="${message.MContent.length() <24  }">${message.MContent}</c:if>
								<span id="com"><a href="javascript:deletemessage(${message.MId})"><img src="images/defind.gif" alt="" width="25" height="25"></a></span>
							  </div>
				     ...
				     <div style="margin-left:300px";>
						<ul>
						   <c:forEach var="i" step="1" begin="1" end="${pages}">
							<li class="active"><a href="findByPageAndUserMessage.action?user.UId=${loginUser.UId}&nowpage=${i}">${i}&nbsp;</a></li>
							</c:forEach>
						</ul>
						</div>
						</center>
