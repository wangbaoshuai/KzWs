...
<html>
  <head>
    ...
    <title>My JSP 'hotellist.jsp' starting page</title>
...
<script type="text/javascript">
	   ...
	   location.href="<%=basePath%>deleteHotel.action?id="+id;
	   }
...
<div id="form">
 <br>
  <table width="1150" border="1">
    ...
    <s:iterator var="ho" value="#session.hotelList" status="i">
    <tr <s:if test="#i.index %2==0 ">style="background:#FFF;"</s:if>>
    <td><img src="hotel_images/<s:property value="#ho.HImage"/>" alt="" width="80" height="50"></td>
    <td><s:property value="#ho.HZip"/></td>
    <td><s:property value="#ho.HName"/></td>
    <td><s:property value="#ho.HContent"/></td>
    <td><s:property value="#ho.HAddress"/></td>
    <td><s:property value="#ho.HTelephone"/></td>
    <td><s:property value="#ho.hcity.cityName"/></td>
    <td><a href="findoneHotel.action?id=<s:property value='#ho.HId'/>" class="ico edit"><span>*修改*</span>|</a>
  <a href="javascript:deletehotel(<s:property value="#ho.HId"/>)" class="ico del">*删除*</a></td>
    </tr>
    </s:iterator>
  </table><br>
  <div class="pagelist">
   <a href="findByPageHotel.action?type=houtai&nowpage=${backpage }">上一页</a>
    <c:forEach var="i" step="1" begin="1" end="${pages}">
    <a href="findByPageHotel.action?type=houtai&nowpage=${i}">${i}&nbsp;&nbsp;</a>
   </c:forEach>
   <a href="findByPageHotel.action?type=houtai&nowpage=${nextpage }">下一页</a>
   <a href="findByPageHotel.action?type=houtai&nowpage=${pages }">末页</a>
     <a>${nowpage}/${pages}页</a> 
   </div>
</div>
</div></div>
