...
<script type="text/javascript">
	 function deletemessage(id){
	   if(confirm("是否确认要删除？"))
	   location.href="<%=basePath%>deleteMessage.action?mid="+id;
	   }
	</script>
...
<table width="1300" border="1">
   ...
   <s:iterator var="message" value="#session.messageList" status="i">
   <tr <s:if test="#i.index %2==0 ">style="background:#FFF;"</s:if>>
   <td><s:property value="#message.MId"/></td>
   <td><s:property value="#message.luser.ULoginname"/></td>
   <td><s:property value="#message.MTitle"/></td>
   <td>
     <c:if test="${message.MContent.length() >= 15 }">${message.MContent.substring(0,15 )}...</c:if>
	 <c:if test="${message.MContent.length() <15 }">${message.MContent}</c:if>
   </td>
   <td><s:property value="#message.MDate"/></td>
   <td><c:choose>
    		<c:when test="${ not empty message.MReply}">已回复</c:when>
    		<c:otherwise>未回复</c:otherwise>
	   	</c:choose>
   </td>
   <td><a href="findoneMessage.action?mid=<s:property value="#message.MId"/>" class="ico edit">*回复*</a>|
       <a href="javascript:deletemessage(<s:property value="#message.MId"/>)" class="ico del">*删除*</a></td>
   </tr> 
   </s:iterator>
   </table>
   <div class="pagelist">
   <a href="findByPageMessage.action?backpage=${backpage }">上一页</a>
   <c:forEach var="i" step="1" begin="1" end="${pages}">
    <a href="findByPageMessage.action?nowpage=${i}">${i}&nbsp;&nbsp;</a>
   </c:forEach>
   <a href="findByPageMessage.action?nextpage=${nextpage }">下一页</a>
   <a href="findByPageMessage.action?pages=${pages }">末页</a>
     <a>${nowpage}/${pages}页</a>
   </div>  
</div>
</div></div>     
