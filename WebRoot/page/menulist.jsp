...
<script type="text/javascript">
	   ...
	   if(confirm("是否确认要删除？"))
	   location.href="<%=basePath%>deleteMenu.action?mid="+mid;
	   }
...
<div id="form">
 <form action="findAllMenu.action?type=houtai" method="post" name="sform">
   <table width="1200" >
   <tr>
   <td><input type="hidden" name="nowpage" value="1"></td>
   <td>菜系名称：</td><td><input type="text" name="menu.MTitle" value="${menu.MTitle}"/></td>
   <td>菜系价格：</td><td><input type="text" name="menu.MPrice" />—<input type="text" name="menu.big_price" /></td>
   <td>菜系类型：</td>
   <td><SELECT class=text  name="menu.menutype.mtId">
...
<table width="1200" border="1">
   ...
   <s:iterator var="men" value="#session.menuList" status="i">
   <tr <s:if test="#i.index %2==0 ">style="background:#FFF;"</s:if>>
   <td><s:property value="#men.MId"/></td>
   <td><s:property value="#men.MTitle"/></td>
   <td><s:property value="#men.MPrice"/></td>
   <td><img src="menu_images/<s:property value="#men.MImg"/>" alt="" width="80" height="40"></td>
   <td><s:property value="#men.menutype.mtName"/></td>
   <td><a href="findoneMenu.action?mid=<s:property value="#men.MId"/>" class="ico edit"><span>*修改*</span>|</a>
  <a href="javascript:deleteMenu(<s:property value="#men.MId"/>)" class="ico del">*删除*</a></td>
   </tr> 
   </s:iterator>
   </table>
