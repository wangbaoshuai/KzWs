...
<html>
  <head>
    ...
    <title>My JSP 'roommodify.jsp' starting page</title>
...
function check(){
   ...
   var bb=document.getElementById("tp").value;
   var cc=document.getElementById("cw").value;
   var ee=document.getElementById("yj").value;
...
<div id="form">
<form action="updateRoom.action" method="post" enctype="multipart/form-data" onsubmit="return check()">
				<table width="800" height="580" border="1">
					 <tr>
						<td>客房编号:</td>
						<td><input type="text"  name="room.RId" value="<s:property value="#session.room.RId"/>" size="50" readonly="readonly"/></td>
					 </tr>
					 <tr>
						<td>客房主题:</td>
						<td><input type="text"  name="room.RTitle" value="<s:property value="#session.room.RTitle"/>" size="50" id="zt"/></td>
					 </tr>
					 <tr>
						<td>客房图片:</td>
						<td><input type="file"  name="img" value="<s:property value="#session.room.RImg" />" size="50" id="tp"/></td>
					 </tr>
					 <tr>
						<td>客房楼层:</td>
						<td><input type="text"  name="room.RFloor"  value="<s:property value="#session.room.RFloor"/>" size="50" readonly="readonly"/></td>
					 </tr>
					 <tr>
						<td>客房床位数:</td>
						<td><input type="text"  name="room.RBednumber" value="<s:property value="#session.room.RBednumber"/>" size="50" id="cw"/></td>
					 </tr>
					
					 <tr> 
					     <td>客房押金:</td>
					     <td><input type="text"  name="room.rdTotal"  value="<s:property value="#session.room.rdTotal"/>" size="50" id="yj"/></td>
					 </tr>
					 <tr> 
					     <td>客房类型:</td>
					 ...
					 <tr>  
					       <td>客房酒店:</td>
					       <td><input type="text" name="room.hotel.HId" value="<s:property  value="#session.room.hotel.HId"/>" readonly="readonly"/></td>
					...
					<tr>
						<td></td>
						<td><label><input type="submit" name="submit" value="修改" size="10" style="background: rgb(50, 180,12); padding: 7px 10px; border-radius: 4px; border: 1px solid rgb(26, 117, 152); border-image: none; color: rgb(255, 255, 255); font-weight: bold;"/></label>
						<a href="findByPageRoom.action?type=houtai&nowpage=1" style="background: rgb(182, 50, 12); padding: 7px 10px; border-radius: 4px; border: 1px solid rgb(26, 117, 152); border-image: none; color: rgb(255, 255, 255); font-weight: bold;"  >取消</a>
